...
export async function retrieveContext(input: { query: string; limit?: number; visibility: KnowledgeVisibility; documentId?: string; threshold?: number }): Promise<RetrievedContext> {
  ...
  const scored = candidates.flatMap((chunk) => { if (!chunk.metadata) return []; const semanticScore = Math.max(...embeddings.map((embedding) => cosineSimilarity(embedding, chunk.embedding as number[]))); const { lexicalScore, exactMatchBonus } = lexicalEvidence(variants.normalizedQuery, chunk.content); const finalScore = semanticScore * RAG_CONFIG.hybridWeights.semantic + lexicalScore * RAG_CONFIG.hybridWeights.lexical + exactMatchBonus * RAG_CONFIG.hybridWeights.exact; return [{ chunkId: String(chunk._id), documentId: String(chunk.documentId), documentTitle: chunk.metadata.documentTitle, content: chunk.content, score: finalScore, semanticScore, lexicalScore, exactMatchBonus, chunkIndex: chunk.chunkIndex, visibility: chunk.metadata.visibility as KnowledgeVisibility }]; }).sort((a,b) => b.score-a.score).slice(0, RAG_CONFIG.hybridCandidateLimit);
  const ranked = scored.filter((chunk) => chunk.score >= threshold).slice(0, limit); const context = buildContext(ranked);
  if (process.env.NODE_ENV !== 'production') console.info('[RAG] hybrid retrieval', { originalQuery: variants.originalQuery, normalizedQuery: variants.normalizedQuery, expandedQueryVariants: variants.expandedQueries, selectedChunkIds: ranked.map((chunk) => chunk.chunkId), scores: ranked.map((chunk) => ({ semanticScore: chunk.semanticScore, lexicalScore: chunk.lexicalScore, exactMatchBonus: chunk.exactMatchBonus, finalScore: chunk.score })) });
