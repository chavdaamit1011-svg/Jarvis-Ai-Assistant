...
export const universalPlanSchema = z.object({
  ...
  filters: z.record(z.string(), z.unknown()), projection: z.array(z.string().max(100)).max(20),
  outputMode: z.enum(['narrative', 'only_requested_fields']),
  references: z.array(z.object({ type: z.string().max(50), value: z.string().max(240) })).max(20),
